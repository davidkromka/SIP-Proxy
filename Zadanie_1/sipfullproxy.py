...
sipstate = {'Ringing': 'Zvoní', 'Trying': 'Skúša', 'Busy here': 'Obsadené', 'Decline': 'Odmietnuté',
            'BYE': 'Dovidenia', 'Accepted': 'Prijaté'}
...
class UDPHandler(socketserver.BaseRequestHandler):
    ...
    def processInvite(self):
        ...
        calls.call(origin, destination, ' volá ')
    ...
    def processNonInvite(self):
        ...
        if len(destination) > 0:
            logging.info("destination %s" % destination)
            if destination in registrar and self.checkValidity(destination):
                calls.call(origin, destination, ' ukončil hovor s ')
                socket, claddr = self.getSocketInfo(destination)
                # self.changeRequestUri()
                self.data = self.addTopVia()
                data = self.removeRouteHeader()
    ...
    def processCode(self):
        ...
        if len(origin) > 0:
            ...
            if origin in registrar:
                ...
                text = "\r\n".join(data)
                word = data[0].split(' ')[2]
                if word in sipstate.keys():
                    text = text.replace(word, sipstate[word])
                    print(text)
                if re.compile("200 Ok").search(text) and re.compile("INVITE").search(text):
                    calls.call(destination, origin, ' prijal hovor od ')
                if re.compile("603 Zložené").search(text):
                    calls.call(destination, origin, ' odmietol hovor od ')
                text = text.encode()
                socket.sendto(text, claddr)
                showtime()
                logging.info("<<< %s" % data[0])
